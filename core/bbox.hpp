...
//            bounds[1] = Vector3F(-INFINITY, -INFINITY, -INFINITY);
        ...
        inline BBOX(/* Vector3F const &p */ Vector3f const &p) {
            bounds[0] = p;
            bounds[1] = p;
        }

        inline BBOX(/* Vector3F const &p1, Vector3F const &p2 */ Vector3f const &p1, Vector3f const &p2) {
            bounds[0] = Min(p1, p2);
            bounds[1] = Max(p1, p2);
        }
...
//        }
        ...
        bool Intersect(Ray const &ray, /* Float *const t_min, Float *const t_max */ float *const t_min,
                       float *const t_max) const;
    };
